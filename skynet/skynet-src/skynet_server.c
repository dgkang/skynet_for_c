...
cmd_reg(struct skynet_context * context, const char * param) {
	if (param == NULL || param[0] == '\0') {
        ...
        struct remote_name rname;
        memset(&rname,0,sizeof(struct remote_name));
        strncpy(rname.name,param,GLOBALNAME_LENGTH - 1);
...
cmd_name(struct skynet_context * context, const char * param) {
	...
	if (name[0] == '.') {
        ...
        strncpy(rname->name,name,GLOBALNAME_LENGTH -1);
