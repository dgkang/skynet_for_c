...
{
   struct lua_seri_handle * h = skynet_malloc(sizeof(struct lua_seri_handle));
   h->b.next = NULL;
   struct write_block *wb = &h->wb;
   wb->current = NULL;
   wb->head = NULL;
   wb->len = 0;
   wb->ptr = 0;
...
void lua_seri_free(struct lua_seri_handle* h)
{
    assert(h != NULL);
    wb_free(&h->wb);
    skynet_free(h);
